...
from Neighbour import UniformNeighborSampler
from Aggregation import MeanAggregator, MaxPoolingAggregator, GatedMeanAggregator
import numpy as np
import Match
...
class Graph2SeqNN(object):
    ...
    def act(self, sess, mode, dict, if_pred_on_dev):
        ...
        feature_info_means = np.array([np.mean(idx) for idx in feature_info]).astype(int)
        feature_info = [np.array([mean]) for mean in feature_info_means]
        ...
        # max_index = np.max(feature_info)
        # print("Max Index:", max_index)
        # print("Indices:", feature_info)
